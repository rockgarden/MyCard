    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (intent.hasExtra("x") && intent.hasExtra("y")) {
            ...
            mMapView = new MapView(this,
                    new BaiduMapOptions().mapStatus(new MapStatus.Builder().target(p).zoom(9).build()));
        } else {
            LatLng p = new LatLng(30.23300678, 120.233899);
            mMapView = new MapView(this, new BaiduMapOptions().mapStatus(new MapStatus.Builder().target(p).zoom(13).build()));
        }
